        ...
        if not seleccion.empty:
            ...
            for _, props in seleccion.iterrows():
                info.append({
                    ...
                    "NOMBREMONT": props.get("NOMBREMONT", "Desconocido"),
...
def generar_imagen_estatica_mapa(x, y, parcela_gdf=None, zoom=16, size=(800, 600)):
    ...
    delta_lon = (size[0] * meters_per_pixel) / (111319.9 * math.cos(math.radians(lat))) * 2.0
    delta_lat = (size[1] * meters_per_pixel) / 111319.9 * 2.0
    ...
    if legend_images:
        ...
        total_legend_height = sum(img.height for img, _ in legend_images) + 30
...
def generar_pdf(datos, x, y, filename, parcela=None):
    ...
    def crear_tabla_mup(afecciones_mup):
        pdf.set_font("Arial", "B", 10)
        col_widths = [30, 60, 50, 40]  # Anchos de columnas: ID, Nombre, Municipio, Propiedad
        headers = ["ID Monte", "Nombre", "Municipio", "Propiedad"]
        ...
        for afeccion in afecciones_mup:
            pdf.cell(col_widths[0], 8, str(afeccion["ID_MONTE"]), border=1)
            pdf.cell(col_widths[1], 8, str(afeccion["NOMBREMONT"]), border=1)
            pdf.cell(col_widths[2], 8, str(afeccion["MUNICIPIO"]), border=1)
            pdf.cell(col_widths[3], 8, str(afeccion["PROPIEDAD"]), border=1)
            pdf.ln()
    ...
    afecciones_keys = [k for k in datos if k.lower().startswith("afección") and k != "afección MUP"]
    ...
    if afecciones_keys:
        ...
        pdf.set_font("Arial", "", 12)
        pdf.cell(0, 8, "No se han detectado otras afecciones.", ln=True)
...
if submitted:
    if not nombre or not apellidos or not dni or x == 0 or y == 0:
        ...
        afecciones = [afeccion_enp, afeccion_zepa, afeccion_lic, afeccion_vp, afeccion_tm]
        if afeccion_mup:
            afecciones.append("Ver tabla de afecciones MUP a continuación")
